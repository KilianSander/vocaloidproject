...
emotional_baseline <-
  ...
  tibble::add_case(
    tibble::tibble(
      ...
      ja = "次に、6枚の写真を順にお見せします。それぞれの写真がどの程度ポジティブ／ネガティブな表現を表すか、また興奮／リラックスした状態であるかを、画像スケールを使って評価してください。"
    )
  )
  ...
  tibble::add_case(
    tibble::tibble(
      ...
      de = paste0(
        ...
        "Solltest Du das nicht machen, wird das Experiment abgebrochen und Du wirst zu Prolific zurückgeleitet und von der Studie ausgeschlossen (<q>rejection</q>).</p>",
        "<p>",
        "Wenn Du alle drei Sitzungen erfolgreich abgeschlossen hast, erhältst Du zusätzlich eine Bonuszahlung.",
        "</p>",
        "<p>Deine Teilnahme ist freiwillig. Voraussetzung zur Teilnahme ist, dass Du <b>18 Jahre oder älter bist, Deutsch Deine Muttersprache ist und Du KEINE Kenntnisse der Japanischen Sprache hast</b>. ",
        "Du kannst Deine Teilnahme jederzeit beenden ohne Angabe von Gründen oder negative Konsequenzen. Bei vorzeitigem Abbruch werden Deine Daten gelöscht.</p>",
        "<p>Alle Informationen, die wir durch die Befragung von Dir erhalten, werden vertraulich behandelt und sind anonymisiert, d.h. können in keiner Weise zu Deiner Person zurückverfolgt werden. ",
      ...
      de_f = paste0(
        ...
        "Sollten Sie das nicht machen, wird das Experiment abgebrochen und Sie werden zu Prolific zurückgeleitet und von der Studie ausgeschlossen (<q>rejection</q>).</p>",
        "<p> ",
        "Wenn Sie alle drei Sitzungen erfolgreich abgeschlossen haben, erhalten Sie zusätzlich eine Bonuszahlung.",
        "</p>",
        "<p>Ihre Teilnahme ist freiwillig. Voraussetzung zur Teilnahme ist, dass Sie <b>18 Jahre oder älter sind, Deutsch Ihre Muttersprache ist und Sie KEINE Kenntnisse der Japanischen Sprache haben</b>. ",
        "Sie können Ihre Teilnahme jederzeit beenden ohne Angabe von Gründen oder negative Konsequenzen. Bei vorzeitigem Abbruch werden Ihre Daten gelöscht.</p>",
        "<p>Alle Informationen, die wir durch die Befragung von Ihnen erhalten, werden vertraulich behandelt und sind anonymisiert, d.h. können in keiner Weise zu Ihrer Person zurückverfolgt werden. ",
      ...
      ja = paste0(
        "<p>本研究へ興味をお持ちいただき、誠にありがとうございます。</p>",
        "<p>本研究は、ポピュラー音楽における感情表現を調査することを目的としています。ご参加の際には、<b>45分程度のセッションを３つ</b>完了していただきます。",
        "各セッションは、参加者ご自身のことや音楽経験に関する質問から始まり、その後合計40の楽曲の一部を評価していただきます。回答に正解や不正解はありません。</p><p>", # additional p-tag (compared to de and en)
        "すべての音楽サンプルを集中して注意深くお聞きいただくことが重要です。",
        "そうしなかった場合、実験は途中で終了し、PROLIFIC（プロリフィック）に戻されてしまいます。</p><p>", # additional p-tag (compared to de and en)
        "この場合、研究からは除外され、今回のセッションに対する報酬は支払われず、次回以降のセッションにも参加することはできません。</p>",
        "<p>",
        "３つすべてのセッションを完了した場合には、追加でボーナス報酬が支払われます。",
        "</p>",
        "<p>研究へのご参加は任意です。</p><p>参加条件は、<b>18歳以上で、母語が日本語であること</b>です。", # additional p-tag (compared to de and en)
        "ご参加はいつでも理由を問わず中止することができ、その際にもみなさまに何ら不利益はありません。途中で中止された場合、提供いただいたデータは削除されます。</p>",
        "<p>本研究で取得したすべての情報は機密として扱われ、匿名化されます。つまり、個人が特定されることは一切ありません。",
        "データの収集および処理は、名前や個人を特定できる情報なしで行われます。",
        "匿名化されたデータは基礎研究に利用され、学術誌への掲載や学会での発表に使用されるほか、科学研究の透明性と再現性を支援するために、データベースとしての公開を予定しています。</p>",
        "<p>ご不明点やお問い合わせは、以下にご連絡ください。研究担当者：保子 英之（<a href='mailto:hhoshi@fc.ritsumei.ac.jp'>hhoshi@fc.ritsumei.ac.jp</a>）</p>"
      )
    )
  ) |>
  tibble::add_case(
...
HALT_dict <-
  ...
  dplyr::left_join(
    tibble::tribble(
      ~key, ~ja,
  #     "AGAIN", "",
      "DEVICE_CHOICE1", "ヘッドホンやイヤホン",
      "DEVICE_CHOICE2", "ノートパソコンの内蔵スピーカー",
      "DEVICE_CHOICE3", "据え置き型スピーカー",
      "DEVICE_CHOICE4", "スマートフォンの内蔵スピーカー",
      "DEVICE_CHOICE5", "タブレットの内蔵スピーカー",
      "DEVICE_CHOICE6", "モニター／テレビの内蔵スピーカー",
      "DEVICE_PROMPT", "<p>現在、音声を出力するために、どのデバイスを使用していますか。</p><p>例： スマートフォンから再生される音声をイヤホンで聴いている場合は、「ヘッドホンやイヤホン」を選択してください。</p><p>ご使用のデバイスに最も当てはまる選択肢を選んでください。</p>",
  #     "INTRO_TEXT", "",
  #     "PAGE_COUNTER", "",
  #     "SCC_PROMPT_HP", "",
  #     "SCC_PROMPT_LS", "",
      "STOP_HEAD", "参加者 様",
      "STOP_TEXT", "<p>アンケートは終了しました。終了の理由は以下のいずれかです：<ol><li>同様の再生デバイスを持つ参加者の数はすでに十分であること。</li><li>ご利用の再生機器は、現在の調査対象に含まれていないこと。</li></ol>今後の調査では、より多くの被験者や他の対象グループを求められるようになる可能性があるため、次回の調査でもまたご参加いただければ幸いです。</p><p>ご協力とご参加、誠にありがとうございました。</p>",
      "TESTNAME", "ヘッドホンやスピーカーの確認",
      "TESTNAME_SHORT", "HALT",
